...
func getSimulateFromSeedInput(tb testing.TB, w io.Writer, app *BitcoivaApp) (
...
func testAndRunTxs(app *BitcoivaApp) []simulation.WeightedOperation {
	cdc := MakeCodec()
...
func invariants(app *BitcoivaApp) []sdk.Invariant {
...
// Profile with:
// /usr/local/go/bin/go test -benchmem -run=^$ github.com/BITCOIVA/Bitcoiva-sdk/BitcoivaApp -bench ^BenchmarkFullAppSimulation$ -Commit=true -cpuprofile cpu.out
func BenchmarkFullAppSimulation(b *testing.B) {
	...
	app := NewBitcoivaApp(logger, db, nil, true, 0)
...
func TestFullAppSimulation(t *testing.T) {
	...
	app := NewBitcoivaApp(logger, db, nil, true, 0, fauxMerkleModeOpt)
	require.Equal(t, "BitcoivaApp", app.Name())
...
func TestAppImportExport(t *testing.T) {
	...
	newApp := NewBitcoivaApp(log.NewNopLogger(), newDB, nil, true, 0, fauxMerkleModeOpt)
...
func TestAppSimulationAfterImport(t *testing.T) {
	...
	newApp := NewBitcoivaApp(log.NewNopLogger(), newDB, nil, true, 0, fauxMerkleModeOpt)
	require.Equal(t, "BitcoivaApp", newApp.Name())
