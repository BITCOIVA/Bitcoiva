...
`bitcoivacli`는 코스모스 테스트넷에서 이루어지는 트랜잭션과 계정을 관리하는 커맨드 라인 인터페이스입니다. 다음은 유용할 수 있는 `bitcoivacli` 명령어입니다:

`bitcoivacli`의 설정 파일은 `$HOME/.bitcoivacli/config/config.toml` 경로에 저장되며, 파일 수정 또는 `bitcoivacli config` 명령어를 통해 수정할 수 있습니다:

```bash
bitcoivacli config chain-id bitcoiva-9004
```

명령어 사용에 관련한 정보는 help를 참고하세요: `bitcoivacli config --help`.

이 문서에는 유용한 `bitcoivacli` 명령어와 예시가 포함되어있습니다.
...
- `cosmos`
  - `bitcoivacli keys add`로 생성되는 계정 키
  - 자금을 받는데 사용
...
- `cosmosvalconspub`
  - `bitcoivad init`로 새로운 노드가 생성될때 같이 생성되는 키.
  - `bitcoivad tendermint show-validator` 명령으로 키 값을 확인할 수 있음
...
```bash
bitcoivacli keys add --multisig=name1,name2,name3[...] --multisig-threshold=K new_key_name
```
...
```bash
bitcoivacli keys add --multisig=foo,bar,baz --multisig-threshold=2 multisig_address
bitcoivacli keys add --multisig=baz,foo,bar --multisig-threshold=2 multisig_address
```

멀티시그 키의 주소는 다음과 같이 빠르게 생성하여 커맨드라인에 프린트할 수 있습니다:

```bash
bitcoivacli keys show --multisig-threshold K name1 name2 name3 [...]
```
...
```bash
bitcoivacli tx send ... --fees=100photino
```

또는

```bash
bitcoivacli tx send ... --gas-prices=1uatom
```
...
```bash
bitcoivacli query account <account_cosmos(코스모스 주소)>
```
...
```bash
bitcoivacli tx send <destination_cosmos(수신자 코스모스 주소)> 10faucetToken \
...
```bash
bitcoivacli query account <account_cosmos(보낸 코스모스 계정 주소)>
bitcoivacli query account <destination_cosmos(수신자 코스모스 주소)>
```

특정 블록 높의에서의 잔고를 확인하고 싶으시다면 `--block` 플래그를 사용하실 수 있습니다:

```bash
bitcoivacli query account <account_cosmos(코스모스 코스모스 주소)> --block=<block_height(블록 높이)>
```

트랜잭션을 실제 전파하지 않고 시뮬레이션을 하시려면 명령어 뒤에 `--dry-run` 플래그를 추가하세요:

```bash
bitcoivacli tx send <destination_cosmosaccaddr(받는이 계정 주소)> 10faucetToken \
...
```bash
bitcoivacli tx send <destination_cosmosaccaddr(받는이 코스모스 주소)> 10faucetToken \
  --chain-id=<chain_id(체인 아이디)> \
  --from=<key_name(보내는 키/계정 이름)> \
...
```bash
bitcoivacli tx sign --validate-signatures signedSendTx.json
```

서명된 트랜잭션을 노드로 전파하기 위해서는 JSON 파일을 다음 명령어를 통해 전달하면 됩니다:

```bash
bitcoivacli tx broadcast --node=<node> signedSendTx.json
```
...
```bash
bitcoivacli query txs --events='message.sender=cosmos1...'
```

다수의 `events`를 이용하실 경우:

```bash
bitcoivacli query txs --events='message.sender=cosmos1...&message.action=withdraw_delegator_reward'
```

페이지네이션은 `page`와 `limit` 값으로 지원됩니다.

```bash
bitcoivacli query txs --events='message.sender=cosmos1...' --page=1 --limit=20
```
...
- [Staking events](https://github.com/BITCOIVA/Bitcoiva-sdk/blob/master/x/staking/spec/07_events.md)
- [Governance events](https://github.com/BITCOIVA/Bitcoiva-sdk/blob/master/x/gov/spec/04_events.md)
- [Slashing events](https://github.com/BITCOIVA/Bitcoiva-sdk/blob/master/x/slashing/spec/06_events.md)
- [Distribution events](https://github.com/BITCOIVA/Bitcoiva-sdk/blob/master/x/distribution/spec/06_events.md)
- [Bank events](https://github.com/BITCOIVA/Bitcoiva-sdk/blob/master/x/bank/spec/04_events.md)
:::
...
```bash
bitcoivacli tx slashing unjail --from <validator-operator-addr(검증인 오퍼레이터 주소)>
```
...
```bash
bitcoivacli query slashing signing-info <validator-pubkey(검증인 pubkey)>
```
...
```bash
bitcoivacli query slashing params
```
...
```bash
bitcoivacli query staking validator <account_cosmosval(cosmosval 계정)>
```
...
```bash
bitcoivacli tx staking delegate \
...
```bash
bitcoivacli keys show [name] --bech val
```

여기에서`[name]`은 `bitcoivad`를 처음 설정하셨을때 정의한 키의 명칭입니다.
...
```bash
bitcoivacli query staking delegation <delegator_addr(위임자 코스모스 주소)> <validator_addr(검증인 주소)>
...
```bash
bitcoivacli query staking delegation <delegator_addr(위임자 코스모스 주소)>
```
...
```bash
bitcoivacli query staking unbonding-delegation <delegator_addr(위임자 주소)> <validator_addr(검증인 주소)>
```

또는 특정 위임자의 모든 언본딩 정보를 확인하고 싶으신 경우:

```bash
bitcoivacli query staking unbonding-delegations <account_cosmos(위임자 주소)>
```

추가적으로 특정 검증인으로 부터 언본딩하는 정보를 확인하고 싶으신 경우:

```bash
bitcoivacli query staking unbonding-delegations-from <account_cosmosval(검증인 cosmosval 주소)>
```
...
```bash
bitcoivacli tx staking redelegate \
...
```bash
bitcoivacli query staking redelegation <delegator_addr(위임자 주소)> <src_val_addr(위임 취소한 검증인 주소)> <dst_val_addr(위임 받을 검증인 주소)>
```

특정 위임자의 모든 검증인에 대한 재위임을 확인하고 싶으신 경우:

```bash
bitcoivacli query staking redelegations <account_cosmos(위임자 코스모스 주소)>
```

특정 검증인에 대한 재위임을 확인하고 싶으신 경우:

```bash
  bitcoivacli query staking redelegations-from <account_cosmosval(검증인 주소)>
```
...
```bash
bitcoivacli query staking params
```
...
```bash
bitcoivacli query staking pool
```
...
```bash
  bitcoivacli query delegations-to <account_cosmosval(검증인 주소)>
```
...
```bash
bitcoivacli tx gov submit-proposal \
  --title=<title(프로포절 제목)> \
...
```bash
bitcoivacli query gov proposal <proposal_id>
```

모든 프로포절에 대한 조회를 하기 위해서는:

```bash
bitcoivacli query gov proposals
```
...
```bash
bitcoivacli query gov proposer <proposal_id(프로포절 ID)>
```
...
```bash
bitcoivacli tx gov deposit <proposal_id(프로포절 ID)> <200000000uatom(금액)> \
  --from=<name(트랜잭션을 발생시킬 키/계정 이름)> \
  --chain-id=<chain_id(체인 아이디)>
```
...
```bash
bitcoivacli query gov deposits <proposal_id(프로포절 ID)>
```

특정 주소에 대한 보증금은 다음과 같이 확인하실 수 있습니다:

```bash
bitcoivacli query gov deposit <proposal_id(프로포절 ID)> <depositor_address(예치자 주소)>
```
...
```bash
bitcoivacli tx gov vote <proposal_id> <Yes/No/NoWithVeto/Abstain(표 선택)> \
  --from=<name(트랜잭션을 발생시킬 키/계정 이름)> \
  --chain-id=<chain_id(체인 아이디)>
```
...
```bash
bitcoivacli query gov vote <proposal_id(프로포절 ID)> <voter_address(투표자 주소)>
```
과거 프로포절에 대한 표 정보를 확인하기 위해서는:

```bash
bitcoivacli query gov votes <proposal_id(프로포절 ID)>
```
...
```bash
bitcoivacli query gov tally <proposal_id(프로포절 ID)>
```
...
```bash
bitcoivacli query gov param voting
bitcoivacli query gov param tallying
bitcoivacli query gov param deposit
```
...
```bash
bitcoivacli query distribution params
```
...
```bash
bitcoivacli query distribution outstanding-rewards
```
...
```bash
bitcoivacli query distribution commission <validator_address(검증인 주소)>
```
...
```bash
bitcoivacli query distribution slashes <validator_address(검증인 주소)> <start_height(시작 블록 높이)> <end_height(마지막 블록 높이)>
```
...
```bash
bitcoivacli query distribution rewards <delegator_address(위임자 주소)> <validator_address(검증인 주소)>
```
...
```bash
bitcoivacli query distribution rewards <delegator_address(위임자 주소)>
```
...
```bash
bitcoivacli keys show --address p1p2p3
```

위 주소를 기반으로 멀티시그 트랜잭션을 생성하는 과정의 첫 단계는 다음과 같습니다:

```bash
bitcoivacli tx send cosmos1570v2fq3twt0f0x02vhxpuzc9jc4yl30q2qned 10000000uatom \
...
흔히 사용되는 `Bash`와 `Zsh` 같은 UNIX의 완료 스크립트(completion script)는 `completion` 명령어를 사용해 생성될 수 있습니다. 이 명령은 `bitcoivad`와 `bitcoivacli`에서 사용 가능합니다.

`Bash` 완료 스크립트를 생성하기 위해서는 다음 명령어를 실행하세요:

```bash
bitcoivad completion > bitcoivad_completion
bitcoivacli completion > bitcoivacli_completion
```

`Zsh` 완료 스크립트를 생성하기 위해서는 다음 명령어를 실행하세요:

```bash
bitcoivad completion --zsh > bitcoivad_completion
bitcoivacli completion --zsh > bitcoivacli_completion
```
...
```bash
echo '. bitcoivad_completion' >> ~/.bashrc
echo '. bitcoivacli_completion' >> ~/.bashrc
```
