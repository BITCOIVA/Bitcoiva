...
`bitcoivacli` is the tool that enables you to interact with the node that runs on the Cosmos Hub network, whether you run it yourself or not. Let us set it up properly. In order to install it, follow the [installation procedure](./installation.md).

### Setting up bitcoivacli 

The main command used to set up `bitcoivacli` is the following:
...
- `cosmos`
  - Derived from account keys generated by `bitcoivacli keys add`
...
- `cosmosvalconspub`
  - Generated when the node is created with `bitcoivad init`.
  - Get this value with `bitcoivad tendermint show-validator`
...
When broadcasting transactions, `bitcoivacli` accepts a `--broadcast-mode` flag. This
...
```bash
bitcoivacli tx send <sender_key_name_or_address> <recipient_address> 10faucetToken \
  --chain-id=<chain_id>
```
...
```bash
bitcoivacli tx send <sender_key_name_or_address> <destination_cosmosaccaddr> 10faucetToken \
  --chain-id=<chain_id> \
  --dry-run
```
...
```bash
bitcoivacli tx send <sender_address> <recipient_address> 10faucetToken \
  --chain-id=<chain_id> \
...
::: tip Note
The `--generate-only` flag prevents `bitcoivacli` from accessing the local keybase.
...
```bash
bitcoivacli query minting params
```

To query for the current inflation value:

```bash
bitcoivacli query minting inflation
```

To query for the current annual provisions value:

```bash
bitcoivacli query minting annual-provisions
```
...
where `[name]` is the name of the key you specified when you initialized `bitcoivad`.
...
```bash
bitcoivacli query distribution community-pool
```
...
```bash
bitcoivacli tx send cosmos1570v2fq3twt0f0x02vhxpuzc9jc4yl30q2qned 1000000uatom \
