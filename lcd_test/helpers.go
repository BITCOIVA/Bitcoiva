...
import (
	...
	"github.com/BITCOIVA/Bitcoiva-sdk/client/lcd"
	...
	gapp "github.com/cosmos/bitcoiva/app"
)
...
func InitializeLCD(nValidators int, initAddrs []sdk.AccAddress, minting bool, portExt ...string) (
	...
	db := dbm.NewMemDB()
	app := gapp.NewBitcoivaApp(logger, db, nil, true, 0, baseapp.SetPruning(store.PruneNothing))
	cdc = gapp.MakeCodec()
...
func startTM(
	...
	privVal tmtypes.PrivValidator, app *gapp.BitcoivaApp,
) (*nm.Node, error) {
...
// NOTE: If making updates here also update cmd/bitcoiva/cmd/bitcoivacli/main.go
